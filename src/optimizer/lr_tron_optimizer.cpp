...
double uTMv(int n, double *u, double *M, double *v) {
  ...
  int i;
  for (i = 0; i < m; i += 5)
    res += u[i] * M[i] * v[i] + u[i + 1] * M[i + 1] * v[i + 1] +
           u[i + 2] * M[i + 2] * v[i + 2] + u[i + 3] * M[i + 3] * v[i + 3] +
           u[i + 4] * M[i + 4] * v[i + 4];
  for (; i < n; i++)
    res += u[i] * M[i] * v[i];
  return res;
...
LRTronOptimizer::LRTronOptimizer(const double *y, const double *z,
                                 int dimension, double rho, int max_iterations,
                                 double epsilon, double cg_epsilon,
...
LRTronOptimizer::~LRTronOptimizer() { delete[] D_; }

void LRTronOptimizer::Optimize(double *x) {
  LOG(DEBUG) << "Start calculate x" << std::endl;
  ...
  while (iter <= max_iterations_ && search) {
    ...
    if (actred < eta0 * prered)
      ...
      delta =
          std::max(sigma1 * delta, std::min(alpha * sMnorm, sigma2 * delta));
    else if (actred < eta2 * prered)
      delta =
          std::max(sigma1 * delta, std::min(alpha * sMnorm, sigma3 * delta));
    else {
      if (reach_boundary)
        delta = sigma3 * delta;
      else
...
int LRTronOptimizer::trpcg(double delta, double *g, double *M, double *s,
                           double *r, bool *reach_boundary) {
  int i, inc = 1;
  double one = 1;
...
double LRTronOptimizer::function_value(const double *x) {
  ...
  for (int i = 0; i < sample_num; ++i) {
    sum += std::log(
        1 + exp(-dataset_->GetLabel(i) * Dot(x, dataset_->GetSample(i))));
  }
  return sum;
}
...
void LRTronOptimizer::gradient(const double *x, double *g) {
  ...
  for (int i = 0; i < sample_num; ++i) {
    double temp =
        Sigmoid(dataset_->GetLabel(i) * Dot(x, dataset_->GetSample(i)));
    D_[i] = temp * (1 - temp);
