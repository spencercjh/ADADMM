...
SparseDataset::SparseDataset(const std::string &file_path) : file_path_(file_path), all_feature_num_(0),
    ...
    std::string temp_str;
    std::string line;
    std::stringstream line_reader;//stringstream 主要是用在將一個字串分割
    while (std::getline(file_reader, line)) {
        ...
        if (line.empty()) {
            ...
            while (line_reader >> temp_str) {
                ...
                size_t idx2;
                /* 尝试进行类型转换，检查是否有错误 */
                std::stoi(index, &idx1);//C++的字符处理函数，把数字字符串转换成int输出
                std::stod(value, &idx2);
    ...
    // 让文件读取指针归位
    file_reader.clear();
    file_reader.seekg(0, std::ifstream::beg);//设置输入文件流的文件流指针位置
    int i = 0, j = 0;
    while (std::getline(file_reader, line)) {
        Trim(line);
        //line.erase(0, line.find_first_not_of(' '));
        ...
        if (line.empty()) {
            ...
            line_reader.clear();//先用 clear( )以及 str( ) 將指定字串設定成一开始的內容
            line_reader.str(line);
